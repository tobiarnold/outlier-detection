...
def main():
    ...
    df_show = df.style.set_properties(**{"background-color": " #ffffa1"}, subset=slice_).format({"height_m": "{:,.1f}","weight_kg": "{:,.1f}"})
    st.dataframe(df_show)
    st.markdown("""----""")
    st.title("📊 Diagramme")
    try:
        config = {"displayModeBar": False}
        fig = px.histogram(df, x="is_legendary", color="is_legendary",title="Verteilung der Klasse is_legendary (Legendäre Pokemons)", color_discrete_map = {0:"lightskyblue",1:"firebrick"})
    ...
    fig = px.imshow(df_corr, aspect="auto",title="Heatmap der Korrelationen",text_auto=True, color_continuous_scale="viridis")
